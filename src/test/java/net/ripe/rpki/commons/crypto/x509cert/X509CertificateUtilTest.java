...
public class X509CertificateUtilTest {
    ...
    @Test
    public void shouldDetermineWhetherCertificateIsSelfSigned() {
        X509ResourceCertificate selfSigned = X509ResourceCertificateTest.createSelfSignedCaResourceCertificate();
        X509ResourceCertificate issued = X509ResourceCertificateTest.createSelfSignedCaResourceCertificateBuilder()
                .withSubjectDN(new X500Principal("CN=child"))
                .withPublicKey(SECOND_TEST_KEY_PAIR.getPublic())
                .build();

        assertTrue(X509CertificateUtil.isSelfSigned(selfSigned.getCertificate()));
        assertFalse(X509CertificateUtil.isSelfSigned(issued.getCertificate()));
    }
